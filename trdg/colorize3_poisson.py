    ...
    def sample_from_data(self, bg_mat):
        ...
        if nn < self.ncol:
            ...
            if diff < self.gray_diff_threshold:
                ...
                r, g, b = bg_col
                # print(r, g, b)
                if math.sqrt(0.241 * r**2  + 0.691 * g**2  + 0.068* b**2 ) > 130:
            ...
            if diff < self.gray_diff_threshold:
                ...
                r, g, b = bg_col
                # print(r, g, b)
                if math.sqrt(0.241 * r**2  + 0.691 * g**2  + 0.068* b**2 ):
                    fg_col = np.array([0, 0, 0])
                else:
                    fg_col = np.array([255, 255, 255])
...
if __name__ == "__main__":
    colorize = Colorize()
    bg_arr = cv2.imread("masks/hiking_125.jpg1603003065735_original.png")
    bg_arr = cv2.cvtColor(bg_arr, cv2.COLOR_BGR2RGB)
    font_color = FontColor(col_file=osp.join('data', 'models/colors_new.cp'))
    fg_col, bg_col = font_color.sample_from_data(bg_arr)
    print(fg_col, bg_col)
